...
const mapStateToProps = state => {
  return {
    incidence: state.quickStats.data.incidence,
...
class Incidence extends Component {
  ...
  componentDidMount(){
     ...
     let i = 0;
     for (const [key, value] of Object.entries(this.props.incidence.charts)) {
  ...
  render() {
    let { incidence } = this.props;
    let { charts } = incidence ? incidence : [];
    let { regions, years } = this.state || {};
    // preparing worldwide data and stored in worldWideObj
    let worldWideObj = incidence && incidence.hasOwnProperty("worldwide") ? { y: Number(incidence.worldwide.replace(/[^0-9\.]+/g, "")), region_name : 'Worldwide' } : null;
    let customizeIncidence = regions.map((data) => {
    ...
    barChartConfig.series[0].data = customizeIncidence;
    return (
      <View style={{flex:1}}>
        <View style={{flex:3}}>
        <ScrollView style={{ flex: 1 }}>
          <Text style={styles.txtStyle}>
            {`Reduction in malaria case incidence rate by WHO region. (${years})`}
          </Text>
            {
...
export default connect (mapStateToProps)(Incidence);