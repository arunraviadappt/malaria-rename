...
class CompareInitiativeCountries extends Component {
  static navigationOptions = () => ({
    title: "Compare Initiative countries"
  });
  ...
  compareCountries = () => {
    if (this.state.selectedCountries.size <= this.state.maxSelection) {
      ...
      this.props.navigation.navigate('CompareInitiativeIndicator')
    }
  }
...
const mapStateToProps = ({ initiatives }) => {
  return {
    countries: initiatives.data[0].countries
  }
};

const mapDispatchToProps = (dispacth) => ({
  compareCountries: (countries) => dispacth(creator.compareCountriesByInitiativeIndicator(countries))
})

export default connect(mapStateToProps, mapDispatchToProps)(CompareInitiativeCountries);
